...
router.post('/', async (req, res, next) => { try { const product = { id: `product-${Date.now()}`, ...req.body }; if (isMongoReady()) await Product.create(product); else { const data = await readStore(); data.products.push(product); await writeStore(data) } res.status(201).json(product) } catch (e) { next(e) } })
